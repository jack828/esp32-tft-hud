#ifndef RING_METER__H
#define RING_METER__H

#include <TFT_eSPI.h>
#define TFT_GREY 0x2104

// void drawAlert(TFT_eSPI *tft, int x, int y, int side, bool draw) {
  // if (draw && !range_error) {
    // drawIcon(tft, alert, x - alertWidth / 2, y - alertHeight / 2, alertWidth,
             // alertHeight);
    // range_error = 1;
  // } else if (!draw) {
...
//====================================================================================
// This is the function to draw the icon stored as an array in program memory
// (FLASH)
//====================================================================================
...
void drawIcon(TFT_eSPI *tft, const unsigned short *icon, int16_t x, int16_t y,
...
int ringMeter(
  ...
  buf[len] = ' ';
  buf[len + 1] =
      0; // Add blanking space and terminator, helps to centre text too!
  // Set the text colour to default
  tft->setTextSize(1);

  if (value < vmin || value > vmax) {
    // drawAlert(tft, x, y + 90, 50, 1);
  } else {
    // drawAlert(tft, x, y + 90, 50, 0);
  }
  ...
  if (r > 84) {
    // TODO auto font sizes?
    tft->setTextPadding(20 * 3);   // Allow for 3 digits each 55 pixels wide
